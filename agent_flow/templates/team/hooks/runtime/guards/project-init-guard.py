...
Detects whether the current project has been initialized with .agent-flow/ or
.dev-workflow/. If neither exists, automatically runs `agent-flow init
--dev-workflow` to bootstrap the project before any workflow can proceed.
"""
...
def _find_project_root() -> Path | None:
    """Walk up from cwd to find a directory with .agent-flow/ or .dev-workflow/."""
    cwd = Path.cwd()
...
def _run_init(project_dir: Path) -> bool:
    """Run agent-flow init --dev-workflow in the project directory."""
    try:
        ...
        if result.returncode == 0:
            return True
        print(f"[AgentFlow] init failed (exit {result.returncode}): {result.stderr.strip()}", file=sys.stderr)
        return False
    except FileNotFoundError:
        print("[AgentFlow] agent-flow CLI not found. Install: pip install agent-flow", file=sys.stderr)
        return False
...
def main() -> None:
    ...
    print(
        f"<system-reminder>\n"
        f"[AgentFlow] 项目未初始化 — 正在自动执行 agent-flow init --dev-workflow ...\n"
        f"目标目录: {project_dir}\n"
        f"</system-reminder>"
    )
    ...
    if success:
        ...
        print(
            ...
            f"  agent-flow init --dev-workflow\n"
